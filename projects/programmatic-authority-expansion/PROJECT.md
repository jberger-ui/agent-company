---
name: Programmatic Authority Expansion
slug: programmatic-authority-expansion
description: Expand durable topical coverage across target sites using structured routing, entity strategy, and repeatable content operations.
owner: ceo
includes:
  - ../../tasks/weekly-editorial-council/TASK.md
  - ../../tasks/monthly-serp-intelligence-review/TASK.md
---

# Programmatic Authority Expansion

## Objective

Build repeatable content programs that increase authority coverage by topic cluster, entity set, and reader problem.

## Success criteria

- content ideas map to defined topical gaps
- persona framing aligns with business goals
- routed ideas can be batched without losing quality
